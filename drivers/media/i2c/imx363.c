...
static int imx363_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VFLIP:
	case V4L2_CID_HFLIP:
		ret = cci_write(imx363->regmap, IMX363_REG_ORIENTATION,
				(imx363->hflip->val ?
				 IMX363_ORIENT_HFLIP : 0) |
				(imx363->vflip->val ?
				 IMX363_ORIENT_VFLIP : 0),
				NULL);
		break;
	default:
		dev_info(&client->dev,
				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	mode = v4l2_find_nearest_size(imx363_supported_modes,
		ARRAY_SIZE(imx363_supported_modes), width, height,
...
static int imx363_init_controls(struct imx363 *imx363)
{
	...
	imx363->link_freq = v4l2_ctrl_new_int_menu(ctrl_hdlr,
				&imx363_ctrl_ops,
				V4L2_CID_LINK_FREQ,
				ARRAY_SIZE(imx363_link_freq_menu) - 1,
				0, imx363_link_freq_menu);
	if (imx363->link_freq)
	...
	imx363->exposure = v4l2_ctrl_new_std(ctrl_hdlr, &imx363_ctrl_ops,
				...
				IMX363_VTS_MAX - IMX363_EXPOSURE_OFFSET,
